...
INPUT_JSON = BASE_DIR / "output"/"normalized_multi_logs.json"
OUTPUT_ANOMALIES = BASE_DIR / "output" / "flagged_anomalies_3.json"
...
for log in logs:
    path = log["signature"].lower()
    status = log["signature"]
...
unique_flagged = {f"{l['ip']}_{l['timestamp']}_{l['signature']}": l for l in flagged_anomalies}.values()
